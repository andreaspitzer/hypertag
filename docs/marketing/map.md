...
- [GitHub repo topics + social preview](issues/04-github-topics-social-preview.md) — plan-only repo-side surface (a human applies it, no push). New About-box **description** is a tightened sibling of the npm one (link-preview/edge-first, drops the stale parser framing, keeps `fastest`/`edge-ready`/`~5 kB`, 🔗 over the old 🏎). **Topics**: the ticket-02 keyword spine adapted to GitHub browse facets — 13 recommended (`link-preview unfurl opengraph open-graph og-tags twitter-cards json-ld meta-tags html-metadata html-parser cloudflare-workers edge favicon`), dropping the current generic `javascript`/`nodejs`/`tag-parsing`, with a reserve list held back to avoid dilution. **Social card**: two 1280×640 concepts drafted as editable SVG + rendered PNG in `docs/marketing/assets/` — A "The card" (problem-first, shows the output card object) and B "The proof" (wordmark-hero, claims as chips); **human chose A** (B kept as an unused alternate), to be exported to PNG and uploaded. `homepage`→npm noted as an adjacent optional flip.
- [Record the VoC positioning durably](issues/06-positioning-durably.md) — decision recorded as a "Why this positioning" preamble at the top of [`principles.md`](principles.md) (human's call), directly above the rules it justifies: the decision (problem-first, edge link-preview job, pain order edge → bundle → memory, extractor-forward / parser-as-floor, neutral confidence), *why* it is evidence-derived, the two research inputs linked, and the single-platform VoC corpus noted as a non-blocking limit. A `docs/adr/` entry, a marketing-scoped decision record, and a `CONTEXT.md` note were all set aside – positioning is a marketing decision, not architecture, so it belongs beside its rules; `docs/adr/` stays architecture-only.
...
_(none open, and none in fog – every patch has graduated and resolved: "Docs surface for the VoC
framing" into ticket 03, and "Recording the positioning durably" into ticket 06. No open or blocked
tickets remain; the way to the destination is clear. The planned/drafted deliverables (from-scratch
README, `package.json` description + keywords, realigned in-repo docs, GitHub topics + social preview,
and the durable positioning record) are ready for a human to commit and `npm publish` / push.)_
